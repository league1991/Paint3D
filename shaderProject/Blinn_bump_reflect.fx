...
float4 std_PS(vertexOutput IN) : COLOR 
{
    ...
    float3 diffuseColor = float3(0.8,0.5,0);//tex2D(ColorSampler,IN.UV).rgb;
    float3 result = specContrib+(diffuseColor*(diffContrib+AmbiColor));
    float3 R = -reflect(Vn,Nn);
	
    float3 reflColor = Kr * texCUBElod(EnvSampler,float4(R.xyz,blurry)).rgb;
	float  ndv = dot(Nn, Vn);
	
	float  fresnelN_m1_sq = fresnelN - 1;
	fresnelN_m1_sq *= fresnelN_m1_sq;
	float  fresnel4N  =  fresnelN * 4;
	float  fresnelN_a1_sq = fresnelN_m1_sq + fresnel4N;
	float  fresnelK_sq = fresnelK * fresnelK;
	float  fresnel = (fresnelN_m1_sq + fresnel4N * pow(1 - ndv,5) + fresnelK_sq) / (fresnelN_a1_sq + fresnelK_sq);
    result = result*(1-fresnel) + fresnel*diffuseColor*reflColor;
